...
import type { Manual } from '../../routes/downloads/+page.server';
...
export function getManuals(): Manual[] {
	...
	try {
		...
		const categories = fs
			.readdirSync(manualsDir, { withFileTypes: true })
					...
					manuals.push({
						...
						files: {
							quickstart: hasQuickstart
								? `/manuals/${category}/${product}/quickstart.pdf`
								: undefined,
							instructions: hasInstructions
								? `/manuals/${category}/${product}/instructions.pdf`
								: undefined,
						},
					});
				}
