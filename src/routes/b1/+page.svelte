<script lang="ts">
	...
	export let data: PageData = { checkoutAvailable: false };
...
$: {
	...
	const sectionIds: string[] = ['hero','features','power','specs','tuning','engineered','comparison'];
...
function scrollToFeature(index: number) {
	...
	if (child) {
		const scrollLeft = child.offsetLeft - 48;
...
function onFeaturesScroll() {
	...
	for (let i = 0; i < children.length; i++) {
		...
		const cardScrollPosition = card.offsetLeft - 48;
	...
	onMount(() => {
		...
		const opts: IntersectionObserverInit = { root: scrollerRoot, threshold: 0.12 };
		const lineIO = new IntersectionObserver((entries) => {
			for (const e of entries) {
				const el = e.target as HTMLElement;
				if (e.isIntersecting) {
					const siblings = Array.from(el.parentElement?.querySelectorAll('[data-parallax-line]') || []);
			...
			const lines = Array.from(rootForLines.querySelectorAll('[data-parallax-line]')) as HTMLElement[];
			...
			if (slidesNode) {
				slidesIO = new IntersectionObserver((entries) => {
					for (const e of entries) {
						if (e.isIntersecting) {
							const svgs = Array.from(slidesNode.querySelectorAll('svg')) as SVGElement[];
							svgs.forEach(svg => {
								const wRects = Array.from(svg.querySelectorAll<SVGRectElement>('rect[data-target-width]')) as SVGRectElement[];
								wRects.forEach(r => {
									const target = Number(r.getAttribute('data-target-width')) || 0;
									r.setAttribute('width', String(Math.max(2, target)));
								});

								const hRects = Array.from(svg.querySelectorAll<SVGRectElement>('rect[data-target-height]')) as SVGRectElement[];
								hRects.forEach(r => {
									const targetH = Number(r.getAttribute('data-target-height')) || 0;
...
<div class="relative w-full h-screen">
		...
		<ul class="m-0 p-0 flex flex-col gap-[18px] list-none">
			{#each sectionIds as id}
				<li class="relative group">
					...
					>
						<span class="block w-[6px] h-[6px] rounded-full bg-white transition duration-300 opacity-55 group-hover:opacity-100" class:opacity-100={active===id}></span>
						<strong class="absolute left-[-8px] top-1/2 -translate-y-1/2 -translate-x-full bg-[#0a0d11] px-2 py-1 text-[0.65rem] tracking-wide border border-white/15 rounded opacity-0 group-hover:opacity-100 text-white font-semibold uppercase whitespace-nowrap transition duration-200" class:opacity-100={active===id}>{id}</strong>
					</button>
				</li>
			{/each}
		</ul>
	...
	<section id="hero" class="relative h-screen w-screen snap-start flex items-center px-12 lg:px-20 overflow-hidden">
		<div class="max-w-7xl mx-auto w-full flex flex-col justify-center space-y-8">
			<div class="text-left space-y-4 z-10 relative">
				...
				<p class="text-[clamp(32px,5vw,56px)] font-semibold leading-tight text-gray-100 hero-subtitle">a new era for Gizzmo</p>
			</div>
	...
	<div class="relative z-10 w-full h-full px-6">
		<div class="h-full flex flex-col">
					<div class="flex-1 flex flex-col justify-center">
						<h3 class="text-[clamp(36px,5vw,56px)] font-bold text-white mb-12 tracking-tight text-left ml-12">Get the highlights.</h3>
						<div class="relative w-full h-full overflow-hidden">
							<div bind:this={featuresTrack} class="flex items-center overflow-x-auto carousel-track snap-x snap-proximity" tabindex="0" role="listbox" aria-label="Feature cards" style="gap: 1.5rem; padding-left: 3rem; scroll-padding-left: 3rem;" on:scroll={onFeaturesScroll} on:keydown={(e) => {
							...
							{#each featureCards as card, i}
								<div 
									...
									class="flex-shrink-0 snap-start rounded-[2rem] flex flex-col transition-all duration-500 border border-white/10 shadow-[30px_0_60px_-15px_rgba(0,0,0,0.5)] overflow-hidden relative" 
									style="width:clamp(700px,80vw,1100px); height:clamp(600px,70vh,850px);">
									
									<div class="absolute inset-0 w-full h-full bg-zinc-900">
										<img src={card.image} alt={card.title} class="w-full h-full object-cover" />
									</div>
									
									<div class="absolute inset-0 bg-gradient-to-b from-black/20 via-black/50 to-black/90 pointer-events-none"></div>
									
									<div class="relative z-10 h-full p-12 flex flex-col justify-end">
										<div class="space-y-6">
											<div>
												<h3 class="text-[48px] font-black leading-tight text-white mb-4 drop-shadow-2xl">{card.title}</h3>
												<p class="text-xl text-white/95 leading-relaxed drop-shadow-lg">{card.description}</p>
											</div>
											<div class="space-y-3">
												{#each card.features as feature}
													<div class="flex items-start gap-3">
														<div class="w-2 h-2 rounded-full mt-2 flex-shrink-0" style="background-color: {card.color};"></div>
														<p class="text-lg text-white/90 drop-shadow-md">{feature}</p>
													</div>
												{/each}
											</div>
										</div>
				...
				<div class="py-8 flex items-center justify-center">
					<div class="flex items-center justify-center py-2.5 px-4 rounded-full gap-1" style="background: rgba(115, 115, 115, 0.3); backdrop-filter: blur(12px);">
						{#each Array(5) as _, i}
							<div
								aria-label={`Slide ${i + 1}`}
								class="h-2 rounded-full transition-all duration-300"
								class:bg-white={featuresIndex === i}
								style={featuresIndex === i ? 'width: 2rem; background: white;' : 'width: 0.5rem; background: rgba(156, 163, 175, 0.5);'}
							>
							</div>
						{/each}
					</div>
	...
	<section id="power" data-section="power" bind:this={sectionRefs.power} class="min-h-screen w-screen snap-start relative py-32 px-8 flex items-center justify-center" class:animate-in={sectionVisible.power}>
		<div class="max-w-7xl mx-auto">
			<div class="text-center mb-20 stagger-fade-in">
				...
				<p class="text-2xl text-white/60 font-medium max-w-3xl mx-auto">Precision-engineered components working in perfect harmony.</p>
			</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				<div class="bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 hover:bg-zinc-900/70 hover:border-white/20 transition-all hover:scale-[1.02] transform stagger-fade-in">
					...
					<div class="text-sm text-white/60">High-resolution color display with vivid, real-time data visualization</div>
				</div>
				...
				<div class="bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 hover:bg-zinc-900/70 hover:border-white/20 transition-all hover:scale-[1.02] transform stagger-fade-in">
					...
					<div class="text-xl font-semibold text-white mb-2">Billet Alloy</div>
					<div class="text-sm text-white/60">Premium machined construction with anodized finish</div>
				</div>
	...
	<section id="specs" data-section="specs" bind:this={sectionRefs.specs} class="min-h-screen w-screen snap-start relative py-32 px-8 flex items-center justify-center" class:animate-in={sectionVisible.specs}>
		<div class="max-w-6xl mx-auto">
			<div class="text-center mb-20 stagger-fade-in">
				...
				<p class="text-2xl text-white/60 font-medium">Unbox. Install. Dominate.</p>
			</div>
			
			<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
				<div class="bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-[2rem] p-10 stagger-fade-in">
					<h4 class="text-3xl font-bold text-white mb-8">Included Hardware</h4>
					<ul class="space-y-4">
						<li class="flex items-start gap-4">
							<div class="w-2 h-2 rounded-full bg-blue-400 mt-2 flex-shrink-0"></div>
							<div>
								<div class="text-lg font-semibold text-white">B1 Controller Unit</div>
								<div class="text-sm text-white/60">Main boost control module with TFT display</div>
							</div>
						</li>
						<li class="flex items-start gap-4">
							<div class="w-2 h-2 rounded-full bg-purple-400 mt-2 flex-shrink-0"></div>
							<div>
								<div class="text-lg font-semibold text-white">MAC Solenoid</div>
								<div class="text-sm text-white/60">High-performance 3-port boost control valve</div>
							</div>
						</li>
						<li class="flex items-start gap-4">
						...
						<li class="flex items-start gap-4">
							...
							<div>
								<div class="text-lg font-semibold text-white">Mounting Brackets</div>
								<div class="text-sm text-white/60">Universal fit mounting hardware</div>
							</div>
						</li>
						<li class="flex items-start gap-4">
		...
		class:animate-in={sectionVisible.tuning}>
				<div class="relative z-10 w-full max-w-7xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
					<div class="space-y-8 content-slide-left">
						<h3 class="text-[clamp(48px,7vw,72px)] font-bold leading-tight text-white tracking-tight">Speed & RPM<br/>Based Tuning</h3>
						<p class="text-xl text-white/60 leading-relaxed font-medium">Configure boost by conditions. Intelligent performance adaptation. Real-time adjustments for maximum power.</p>
					</div>
	...
	<section 
		...
		class:animate-in={sectionVisible.engineered}>
				<div class="text-center mb-16 z-10 relative px-6">
					...
					<p class="text-xl text-white/50 font-medium">Every detail matters. Every feature counts.</p>
				</div>
		...
		class:animate-in={sectionVisible.comparison}>
				<div class="relative w-full max-w-7xl mx-auto">
					<div class="text-center mb-12">
						<h3 class="text-[clamp(40px,6vw,64px)] font-bold text-white mb-4 tracking-tight">How B1 stacks up.</h3>
						<p class="text-xl text-white/50 font-medium">Compare features. See the difference.</p>
					</div>

					<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
						<div class="bg-zinc-900/40 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 overflow-auto">
							<h4 class="text-3xl font-semibold mb-6 text-white/90">Specs at a glance</h4>
							<table class="w-full table-fixed text-left table-clean">
								...
								<tbody>
									...
									<tr class="border-t border-white/5 hover:bg-white/5 transition-colors">
										<td class="px-3 py-4 align-middle text-white/90 font-semibold text-sm">{row.f}</td>
										{#each row.v as cell}
											{#if row.type === 'boolean'}
												<td class="px-2 py-4 text-center">
												<div class={`h-10 flex items-center justify-center rounded-lg ${cell ? 'bg-emerald-600 text-white' : 'bg-red-600 text-white'}`}>
													{#if cell}
													...
													{:else}
														<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
													{/if}
												</div>
											</td>
											{:else}
												<td class="px-2 py-4 text-center">
												<div class="relative bg-neutral-800 rounded-lg h-10 overflow-hidden">
													<div class="absolute inset-0" style={`width:${Math.round((Number(cell) / (row.max || Math.max(...row.v))) * 100)}%; background: linear-gradient(90deg,#10b981,#f59e0b);`} aria-hidden="true"></div>
													<div class="relative z-10 h-full flex items-center justify-center text-white font-bold text-sm">{cell}</div>
												</div>
											</td>
											{/if}
										{/each}
						...
						<div class="bg-zinc-900/40 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 space-y-8">
							<div>
								<h5 class="text-xl font-semibold text-white mb-6">Price Comparison</h5>
								<div class="space-y-4">
									{#each _priceVals as pv, idx}
										<div>
											<div class="flex justify-between mb-2">
												<span class="text-sm font-medium text-white/70">{idx===0? 'GFB' : idx===1? 'B1' : idx===2? 'Greddy' : idx===3? 'HKS' : 'Plex'}</span>
												<span class="text-sm font-bold text-white">${pv}</span>
											</div>
											<div class="h-8 bg-zinc-800 rounded-lg overflow-hidden">
												<div class="h-full rounded-lg transition-all duration-1000" style={`width: ${(pv / Math.max(..._priceVals)) * 100}%; background: ${_priceColors[idx]};`}></div>
											</div>
										</div>
									{/each}
								</div>
							...
							<div>
								<h5 class="text-xl font-semibold text-white mb-6">Max Boost Pressure (Bar)</h5>
								<div class="space-y-4">
									{#each _maxBarVals as mv, idx}
										<div>
											...
											<div class="h-8 bg-zinc-800 rounded-lg overflow-hidden">
												<div class="h-full bg-blue-500 rounded-lg transition-all duration-1000" style={`width: ${(mv / Math.max(..._maxBarVals)) * 100}%;`}></div>
											</div>
										</div>
									{/each}
								</div>
	...
	<section class="min-h-screen w-screen snap-start relative py-32 px-8 flex items-center justify-center">
...
*/
/* Marquee animation */
@keyframes marqueeOpts { 0% { transform:translateX(0); } 100% { transform:translateX(-50%); } }
@keyframes marqueeScroll { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
.animate-marquee { animation: marqueeScroll 18s linear infinite; }
/* Marquee structural helpers kept minimal (can't fully express mask & drop-shadow via utilities nicely) */
.options-marquee{position:absolute;top:2vh;left:0;width:100%;overflow:hidden;height:7vw;pointer-events:none}
.options-marquee .layer{position:absolute;inset:0;display:flex;align-items:center}
.options-marquee .track{display:flex;width:max-content;animation:marqueeOpts 16s linear infinite;font-size:4vw;font-weight:600;letter-spacing:-1px;line-height:1}
.options-marquee .layer.base span{color:#ffffff;opacity:.9}
.options-marquee .layer.edges{mask:linear-gradient(90deg,#000 0%,#000 14%,transparent 38%,transparent 62%,#000 86%,#000 100%);-webkit-mask:linear-gradient(90deg,#000 0%,#000 14%,transparent 38%,transparent 62%,#000 86%,#000 100%);filter:drop-shadow(0 0 8px rgba(255,0,0,0.5))}
.options-marquee .layer.edges span{color:#ff1515}
.options-marquee span{padding-right:4vw;white-space:nowrap}
...
.b1-ellipse-inner{position:absolute;top:50%;left:50%;width:1200px;height:1200px;translate:-50% -50%;background:radial-gradient(circle at 50% 50%,rgba(136,192,208,0.9),rgba(163,190,140,0.7) 40%,rgba(94,129,172,0) 70%);filter:hue-rotate(var(--huerot,0deg)) blur(140px) brightness(1.2);opacity:.7;mix-blend-mode:screen;transition:opacity .35s ease-out,filter .5s ease-out,transform .8s cubic-bezier(.22,.61,.36,1);will-change:transform,opacity}
...
.options-marquee .track{display:flex;gap:2rem}
...
svg rect{transition:width 900ms cubic-bezier(.22,.8,.32,1)}
...
.options-marquee .layer.edges{opacity:0.95}
...
.content-slide-left {
	...
	transition: opacity 900ms cubic-bezier(0.22, 0.8, 0.32, 1), 
...
.content-slide-right {
	opacity: 0;
	transform: translateX(40px);
	transition: opacity 900ms cubic-bezier(0.22, 0.8, 0.32, 1) 200ms, 
...
.content-fade-up {
	...
	transition: opacity 800ms cubic-bezier(0.22, 0.8, 0.32, 1) 100ms, 
...
.table-clean tbody tr:nth-child(odd){background:rgba(255,255,255,0.03)}
.table-clean tbody tr:hover{background:rgba(255,255,255,0.05)}
.bar-item{transition:all 700ms cubic-bezier(.22,.8,.32,1)}
.bar-item span{font-weight:700; font-size:12px}
